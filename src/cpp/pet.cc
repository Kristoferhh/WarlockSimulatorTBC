...
Pet::Pet(std::shared_ptr<Player> player) : player(player) {
  ...
  mode = player->settings->petIsAggressive ? PetMode::AGGRESSIVE
                                           : PetMode::PASSIVE;
  critMultiplier = 1.5;
  glancingBlowMultiplier =
      1 -
      (0.1 + (player->settings->enemyLevel * 5 - player->level * 5) / 100.0);
  glancingBlowChance = std::max(
      0.0, 6 + (player->settings->enemyLevel * 5 - player->level * 5) * 1.2);
...
void Pet::initialize() {
  ...
  if (pet == PetName::IMP) {
    ...
    if (pet == PetName::SUCCUBUS) {
      spells->LashOfPain =
          std::make_unique<SuccubusLashOfPain>(shared_from_this());
    } else if (pet == PetName::FELGUARD) {
      spells->Cleave = std::make_unique<FelguardCleave>(shared_from_this());
      auras->DemonicFrenzy =
          std::make_unique<DemonicFrenzy>(shared_from_this());
    }
  }
...
void Pet::calculateStatsFromAuras() {
  ...
  if (player->selectedAuras->totemOfWrath) {
    ...
    stats->spellHitChance =
        std::min(99.0, stats->spellHitChance + hitAndCritAmount);
  }
  // todo improved motw
  ...
  if (player->selectedAuras->wrathOfAirTotem &&
      player->settings->hasElementalShamanT4Bonus) {
  ...
  if (player->selectedAuras->faerieFire &&
      player->settings->improvedFaerieFire) {
    stats->meleeHitChance += 3;
  }
  if (player->selectedAuras->exposeWeakness) {
    debuffStats->attackPower +=
        (player->settings->survivalHunterAgility * 0.25 *
         (player->settings->exposeWeaknessUptime / 100.0));
  }
  ...
  if (player->selectedAuras->ferociousInspiration) {
    stats->damageModifier *=
        std::pow(1.03, player->settings->ferociousInspirationAmount);
  }
  ...
  if (petType == PetType::MELEE) {
    ...
    if (player->settings->enemyLevel <= 59) {
      armorMultiplier = 1 - player->settings->enemyArmor /
                                (player->settings->enemyArmor + 400 +
                                 85 * player->settings->enemyLevel);
    } else {
      armorMultiplier = 1 - player->settings->enemyArmor /
                                (player->settings->enemyArmor - 22167.5 +
                                 467.5 * player->settings->enemyLevel);
    }
  }
...
void Pet::calculateStatsFromPlayer(bool announceInCombatLog) {
  stats->stamina =
      baseStats->stamina + buffStats->stamina +
      (0.3 * player->stats->stamina * player->stats->staminaModifier);
  stats->intellect =
      baseStats->intellect + buffStats->intellect +
      (0.3 * player->stats->intellect * player->stats->intellectModifier);
  player->demonicKnowledgeSpellPower =
      ((stats->stamina * stats->staminaModifier) +
       (stats->intellect * stats->intellectModifier)) *
      (0.04 * player->talents->demonicKnowledge);
  baseStats->attackPower = (baseStats->strength + buffStats->strength) *
                               stats->strengthModifier * 2 -
                           20;
  stats->attackPower =
      baseStats->attackPower + buffStats->attackPower +
      debuffStats->attackPower +
      (player->getSpellPower() +
       std::max(player->stats->shadowPower, player->stats->firePower)) *
          0.57;
  stats->agility = baseStats->agility + buffStats->agility;
  stats->meleeCritChance = player->talents->demonicTactics +
                           (stats->agility * stats->agilityModifier) * 0.04 +
                           0.65 + buffStats->meleeCritChance;
  stats->spellPower =
      buffStats->spellPower +
      (player->getSpellPower() +
       std::max(player->stats->shadowPower, player->stats->firePower)) *
          0.15;
  if (petType == PetType::MELEE) {
    ...
    stats->maxMana =
        baseStats->mana + stats->intellect * 11.555 * stats->intellectModifier;
  } else if (pet == PetName::IMP) {
    stats->maxMana =
        baseStats->mana + stats->intellect * stats->intellectModifier * 4.95;
  }
  if (pet == PetName::IMP || pet == PetName::SUCCUBUS) {
    stats->spellCritChance =
        0.0125 * (stats->intellect * stats->intellectModifier) + 0.91 +
        player->talents->demonicTactics + buffStats->spellCritChance +
        (buffStats->spellCritRating / player->critRatingPerPercent);
  }
...
void Pet::reset() {
  ...
  // End Auras
  if (auras->BlackBook != NULL && auras->BlackBook->active)
    auras->BlackBook->fade(true);
  if (auras->DemonicFrenzy != NULL && auras->DemonicFrenzy->active)
    auras->DemonicFrenzy->fade(true);
...
double Pet::getSpellCritChance() { return stats->spellCritChance; }

bool Pet::isCrit(AttackType type) {
  if (type == AttackType::PHYSICAL) {
    return player->getRand() <=
           getMeleeCritChance() * player->critChanceMultiplier;
  } else if (type == AttackType::MAGICAL) {
    return player->getRand() <=
           getSpellCritChance() * player->critChanceMultiplier;
  }

  return false;
}

double Pet::getMeleeHitChance() { return stats->meleeHitChance; }
...
bool Pet::isHit(AttackType type) {
  if (type == AttackType::PHYSICAL) {
    return player->getRand() <=
           getMeleeHitChance() * player->critChanceMultiplier;
  } else {
    return player->getRand() <=
           getSpellHitChance() * player->critChanceMultiplier;
  }
}
...
int Pet::getAttackPower() {
  ...
  double ap = (stats->attackPower - debuffStats->attackPower) *
              stats->attackPowerModifier;
...
void Pet::tick(double t) {
  ...
  // Auras
  if (auras->BlackBook != NULL && auras->BlackBook->durationRemaining > 0)
    auras->BlackBook->tick(t);

  // Spells
  if (spells->Melee != NULL && spells->Melee->cooldownRemaining > 0)
    spells->Melee->tick(t);
  if (spells->Cleave != NULL && spells->Cleave->cooldownRemaining > 0)
    spells->Cleave->tick(t);
  if (spells->Firebolt != NULL && spells->Firebolt->casting)
    spells->Firebolt->tick(t);
  if (spells->LashOfPain != NULL && spells->LashOfPain->cooldownRemaining > 0)
    spells->LashOfPain->tick(t);
  ...
  if (spiritTickTimerRemaining <= 0) {
    ...
    if (fiveSecondRuleTimerRemaining <= 0) {
      if (pet == PetName::IMP) {
        manaGain +=
            ((baseStats->spirit + stats->spirit) * stats->spiritModifier) +
            0.7 * (stats->intellect * stats->intellectModifier) - 258;
      } else if (pet == PetName::FELGUARD || pet == PetName::SUCCUBUS) {
        manaGain += 0.75 * ((baseStats->spirit + stats->spirit) *
                            stats->spiritModifier) +
                    0.62 * (stats->intellect * stats->intellectModifier) - 108;
      }
    }
    ...
    stats->mana =
        std::min(stats->maxMana, stats->mana + static_cast<int>(manaGain));
    if (stats->mana > currentMana && player->shouldWriteToCombatLog()) {
      player->combatLog(name + " gains " + std::to_string(round(manaGain)) +
                        " mana from Mp5/Spirit regeneration (" +
                        std::to_string(currentMana) + " -> " +
                        std::to_string(stats->mana) + ")");
    }
  }
}
