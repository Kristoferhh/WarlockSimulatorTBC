  ...
  if (active && player->shouldWriteToCombatLog()) {
    ...
    if (stats != NULL && stats->shadowPower > 0) {
      if (player->shouldWriteToCombatLog()) {
        ...
        player->combatLog(
            "Shadow Power + " + std::to_string(stats->shadowPower) + " (" +
            std::to_string(currentShadowPower) + " -> " +
            std::to_string(currentShadowPower + stats->shadowPower) + ")");
      }
    ...
    if (stats != NULL && stats->firePower > 0) {
      if (player->shouldWriteToCombatLog()) {
        ...
        player->combatLog("Fire Power + " + std::to_string(stats->firePower) +
                          " (" + std::to_string(currentFirePower) + " -> " +
                          std::to_string(currentFirePower + stats->firePower) +
                          ")");
      }
    ...
    if (stats != NULL && stats->hastePercent > 0) {
      if (player->shouldWriteToCombatLog()) {
        player->combatLog(
            "Haste % * " + std::to_string(stats->hastePercent) + " (" +
            std::to_string((player->stats->hastePercent - 1) * 100) + " -> " +
            std::to_string((player->stats->hastePercent *
    ...
    if (stats != NULL && stats->manaCostModifier > 0) {
      if (player->shouldWriteToCombatLog()) {
        ...
        player->combatLog(
            "Mana Cost Modifier * " +
            truncateTrailingZeros(std::to_string(stats->manaCostModifier), 2) +
            " (" +
            truncateTrailingZeros(std::to_string(currentmanaCostModifier), 2) +
            " -> " +
            truncateTrailingZeros(std::to_string(currentmanaCostModifier *
                                                 stats->manaCostModifier),
                                  2) +
            ")");
      }
  ...
  if (stats != NULL && stats->hastePercent > 0) {
    if (player->shouldWriteToCombatLog()) {
      player->combatLog(
          ...
          std::to_string((player->stats->hastePercent - 1) * 100) + " -> " +
          std::to_string(
              (player->stats->hastePercent / (1 + (stats->hastePercent / 100)) -
               1) *
              100) +
          ")");
    }
    ...
    if (player->shouldWriteToCombatLog()) {
      ...
      player->combatLog(
          ...
          truncateTrailingZeros(
              std::to_string(currentmanaCostModifier / stats->manaCostModifier),
              2) +
          ")");
    }
    player->stats->manaCostModifier /= stats->manaCostModifier;
...
ImprovedShadowBoltAura::ImprovedShadowBoltAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
void ImprovedShadowBoltAura::fade() {
  Aura::fade();
  uptimeSoFar =
      player->combatLogBreakdown.at(name)->uptime / player->totalDuration;
}

CurseOfTheElementsAura::CurseOfTheElementsAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
CurseOfRecklessnessAura::CurseOfRecklessnessAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
ShadowTranceAura::ShadowTranceAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
FlameshadowAura::FlameshadowAura(std::shared_ptr<Player> player)
    : Aura(player) {
  name = "Flameshadow";
  duration = 15;
  procChance = 5;
...
ShadowflameAura::ShadowflameAura(std::shared_ptr<Player> player)
    : Aura(player) {
  name = "Shadowflame";
  duration = 15;
  procChance = 5;
...
SpellstrikeAura::SpellstrikeAura(std::shared_ptr<Player> player)
    : Aura(player) {
  name = "Spellstrike";
  duration = 10;
  procChance = 5;
...
PowerInfusionAura::PowerInfusionAura(std::shared_ptr<Player> player)
    : Aura(player) {
  name = "Power Infusion";
  duration = 15;
...
EyeOfMagtheridonAura::EyeOfMagtheridonAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
SextantOfUnstableCurrentsAura::SextantOfUnstableCurrentsAura(
...
QuagmirransEyeAura::QuagmirransEyeAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
ShiffarsNexusHornAura::ShiffarsNexusHornAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
ManaEtched4SetAura::ManaEtched4SetAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
DestructionPotionAura::DestructionPotionAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
DrumsOfBattleAura::DrumsOfBattleAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
DrumsOfRestorationAura::DrumsOfRestorationAura(std::shared_ptr<Player> player)
    : Aura(player) {
  ...
  if (tickTimerRemaining <= 0) {
    ...
    if (player->shouldWriteToCombatLog()) {
      player->combatLog("Player gains " + std::to_string(manaGained) +
                        " mana from Drums of Restoration (" +
...
AshtongueTalismanOfShadowsAura::AshtongueTalismanOfShadowsAura(
    std::shared_ptr<Player> player)
    : Aura(player) {
...
DarkmoonCardCrusadeAura::DarkmoonCardCrusadeAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
TheLightningCapacitorAura::TheLightningCapacitorAura(
    std::shared_ptr<Player> player)
    : Aura(player) {
...
BandOfTheEternalSageAura::BandOfTheEternalSageAura(
    std::shared_ptr<Player> player)
    : Aura(player) {
...
BladeOfWizardryAura::BladeOfWizardryAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
ShatteredSunPendantOfAcumenAura::ShatteredSunPendantOfAcumenAura(
    std::shared_ptr<Player> player)
    : Aura(player) {
...
RobeOfTheElderScribesAura::RobeOfTheElderScribesAura(
    std::shared_ptr<Player> player)
    : Aura(player) {
...
MysticalSkyfireDiamondAura::MysticalSkyfireDiamondAura(
    std::shared_ptr<Player> player)
    : Aura(player) {
...
AmplifyCurseAura::AmplifyCurseAura(std::shared_ptr<Player> player)
    : Aura(player) {
  name = "Amplify Curse";
  duration = 30;
  setup();
...
WrathOfCenariusAura::WrathOfCenariusAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
ChippedPowerCoreAura::ChippedPowerCoreAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
CrackedPowerCoreAura::CrackedPowerCoreAura(std::shared_ptr<Player> player)
    : Aura(player) {
...
ManaTideTotemAura::ManaTideTotemAura(std::shared_ptr<Player> player)
    : Aura(player) {
