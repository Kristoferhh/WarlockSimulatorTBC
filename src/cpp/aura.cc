...
FlameshadowAura::FlameshadowAura(Player& player) : Aura(player) {
  ...
  duration = 15;
  stats.push_back(ShadowPower(player, 135));
  Setup();
}
...
ShadowflameAura::ShadowflameAura(Player& player) : Aura(player) {
  ...
  duration = 15;
  stats.push_back(FirePower(player, 135));
  Setup();
}
...
QuagmirransEyeAura::QuagmirransEyeAura(Player& player) : Aura(player) {
  ...
  duration = 6;
  stats.push_back(SpellHasteRating(player, 320));
  Setup();
}
...
BloodlustAura::BloodlustAura(Player& player) : Aura(player) {
  ...
  if (player.pet != NULL) {
    stats.push_back(SpellHastePercent(*player.pet.get(), 1.3));
    stats.push_back(MeleeHastePercent(*player.pet.get(), 1.3));
  }
  Setup();
}
...
DrumsOfBattleAura::DrumsOfBattleAura(Player& player) : Aura(player) {
  ...
  duration = 30;
  group_wide = true;
  stats.push_back(SpellHasteRating(player, 80));
  Setup();
}
...
BladeOfWizardryAura::BladeOfWizardryAura(Player& player) : Aura(player) {
  ...
  duration = 6;
  stats.push_back(SpellHasteRating(player, 280));
  Setup();
}
