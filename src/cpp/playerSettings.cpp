...
PlayerSettings::PlayerSettings(Auras* auras, Talents* talents, Sets* sets, CharacterStats* stats, Items* items, int itemId, int metaGemId, bool simmingIntellect, bool simmingSpellPower
  ...
  : auras(auras), talents(talents), sets(sets), stats(stats), items(items), itemId(itemId), metaGemId(metaGemId), simmingIntellect(simmingIntellect), simmingSpellPower(simmingSpellPower)
    , simmingShadowPower(simmingShadowPower), simmingFirePower(simmingFirePower), simmingHitRating(simmingHitRating), simmingCritRating(simmingCritRating), simmingHasteRating(simmingHasteRating)
    , simmingMp5(simmingMp5), isAldor(isAldor), enemyLevel(enemyLevel), enemyShadowResist(enemyShadowResist), enemyFireResist(enemyFireResist), mageAtieshAmount(mageAtieshAmount)
    , totemOfWrathAmount(totemOfWrathAmount), sacrificingPet(sacrificingPet), petIsImp(petIsImp), petIsSuccubus(petIsSuccubus), petIsFelguard(petIsFelguard),ferociousInspirationAmount(ferociousInspirationAmount)
    , improvedCurseOfTheElements(improvedCurseOfTheElements), usingCustomIsbUptime(usingCustomIsbUptime), customIsbUptimeValue(customIsbUptimeValue), improvedDivineSpirit(improvedDivineSpirit)
    , improvedImp(improvedImp), shadowPriestDps(shadowPriestDps), warlockAtieshAmount(warlockAtieshAmount), improvedExposeArmor(improvedExposeArmor), isSingleTarget(isSingleTarget), enemyAmount(enemyAmount)
    , isOrc(isOrc), powerInfusionAmount(powerInfusionAmount), bloodlustAmount(bloodlustAmount), innervateAmount(innervateAmount), enemyArmor(enemyArmor),exposeWeaknessUptime(exposeWeaknessUptime)
    , improvedFaerieFire(improvedFaerieFire), infinitePlayerMana(infinitePlayerMana), infinitePetMana(infinitePetMana), usingLashOfPainOnCooldown(usingLashOfPainOnCooldown), petIsAggressive(petIsAggressive)
    , prepopBlackBook(prepopBlackBook), randomizeValues(randomizeValues), simChoosingRotation(simChoosingRotation), exaltedWithShattrathFaction(exaltedWithShattrathFaction)
