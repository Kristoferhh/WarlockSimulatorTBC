...
DamageOverTime::DamageOverTime(std::shared_ptr<Player> player)
    : player(player) {
  duration = 0;
  tickTimerTotal = 3;
...
void DamageOverTime::apply() {
  ...
  if (player->shouldWriteToCombatLog()) {
    ...
    player->combatLog(name + " " + refreshedOrApplied + " (" +
                      std::to_string(spellPower) + " Spell Power)");
  }
  ...
  // duration.
  if (player->spells->SiphonLife != NULL &&
      name == player->spells->SiphonLife->name) {
    isbActive = !player->settings->usingCustomIsbUptime &&
                player->auras->ImprovedShadowBolt != NULL &&
                player->auras->ImprovedShadowBolt->active;
  }
  // Amplify Curse
  if (((player->spells->CurseOfAgony != NULL &&
        name == player->spells->CurseOfAgony->name) ||
       (player->spells->CurseOfDoom != NULL &&
        name == player->spells->CurseOfDoom->name)) &&
      player->auras->AmplifyCurse != NULL &&
...
double DamageOverTime::getModifier() {
  ...
  if ((school == SpellSchool::SHADOW &&
       ...
       (player->spells->SiphonLife == NULL ||
        name != player->spells->SiphonLife->name)) ||
      (player->spells->SiphonLife != NULL &&
       name == player->spells->SiphonLife->name && isbActive)) {
...
std::vector<double> DamageOverTime::getConstantDamage() {
  ...
  // If the DoT isn't currently active then this.spellPower will be 0, so use
  ...
  if (((player->spells->Corruption != NULL &&
        name == player->spells->Corruption->name) ||
       (player->spells->Immolate != NULL &&
        name == player->spells->Immolate->name)) &&
      player->sets->t5 >= 4) {
  ...
  return std::vector<double>{baseDamage, totalDamage, spellPower, modifier,
                             partialResistMultiplier};
}
...
void DamageOverTime::tick(double t) {
  ...
  if (tickTimerRemaining <= 0) {
    ...
    if (player->spells->Corruption != NULL &&
      ...
      if (player->getRand() <=
          player->talents->nightfall * 2 * player->critChanceMultiplier) {
    ...
    if (player->shouldWriteToCombatLog()) {
      std::string msg =
          name + " Tick " + truncateTrailingZeros(std::to_string(round(dmg))) +
          " (" + truncateTrailingZeros(std::to_string(baseDamage)) +
          " Base Damage - " +
          truncateTrailingZeros(std::to_string(spellPower)) +
          " Spell Power - " +
          truncateTrailingZeros(std::to_string(coefficient), 3) +
          " Coefficient - " +
          truncateTrailingZeros(std::to_string(round(modifier * 10000) / 100),
                                3) +
          "% Damage Modifier - " +
      ...
      if (t5BonusModifier > 1) {
        msg += " - " + std::to_string(round(t5BonusModifier * 10000) / 100) +
               "% Base Dmg Modifier (T5 4pc bonus)";
      }
      msg += ")";
    ...
    if (player->spells->Corruption != NULL &&
        name == player->spells->Corruption->name &&
        player->auras->AshtongueTalismanOfShadows != NULL &&
        player->getRand() <=
            player->auras->AshtongueTalismanOfShadows->procChance *
    ...
    if (player->spells->TimbalsFocusingCrystal != NULL &&
        ...
        player->getRand() <=
            player->spells->TimbalsFocusingCrystal->procChance *
                player->critChanceMultiplier) {
...
CorruptionDot::CorruptionDot(std::shared_ptr<Player> player)
...
UnstableAfflictionDot::UnstableAfflictionDot(std::shared_ptr<Player> player)
    : DamageOverTime(player) {
...
SiphonLifeDot::SiphonLifeDot(std::shared_ptr<Player> player)
    : DamageOverTime(player) {
  name = "Siphon Life";
  duration = 30;
  tickTimerTotal = 3;
...
ImmolateDot::ImmolateDot(std::shared_ptr<Player> player)
    : DamageOverTime(player) {
  name = "Immolate";
  duration = 15;
  tickTimerTotal = 3;
...
CurseOfAgonyDot::CurseOfAgonyDot(std::shared_ptr<Player> player)
    : DamageOverTime(player) {
  name = "Curse of Agony";
  duration = 24;
  tickTimerTotal = 3;
...
double CurseOfAgonyDot::getModifier() {
  ...
  modifier *= (1 * (1 + ((player->talents->shadowMastery * 0.02) +
                         (player->talents->contagion / 100.0) +
                         (player->talents->improvedCurseOfAgony * 0.05))));
  return modifier;
}

CurseOfDoomDot::CurseOfDoomDot(std::shared_ptr<Player> player)
    : DamageOverTime(player) {
  name = "Curse of Doom";
  duration = 60;
  tickTimerTotal = 60;
