        ...
        if (simulationsFinished === itemAmount) {
          ...
          if (itemAmount === 1 && $('#automatically-open-sim-details').children('select').val() === 'yes') {
            ...
            for (const aura in simulationEnd.auraBreakdown) {
              ...
              if (a.count > 0) {
                $('#aura-breakdown-table tbody').append("<tr class='spell-damage-information'><td>" + a.name + '</td><td>' + Math.ceil(a.count / simulationEnd.iterationAmount) + "</td><td><meter value='" + percentUptime + "' min='0' max='100'></meter> " + percentUptime + '%</td></tr>')
              }
            }
            ...
            for (const element in simulationEnd.manaGainBreakdown) {
              ...
              if (e.casts > 0) {
                $('#mana-gain-breakdown-table tbody').append("<tr class='spell-damage-information'><td>" + e.name + "</td><td><meter value='" + percentOfGain + "' min='0' max='100'></meter> " + percentOfGain + '</td><td>' + Math.ceil(e.casts / simulationEnd.iterationAmount) + '</td><td>' + ~~(e.manaGain / e.casts) + '</td<</tr>')
              }
            }*/
