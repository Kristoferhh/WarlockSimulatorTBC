...
function disableAurasWithUniqueProperties(
  ...
  if (aura.foodBuff)
    Auras.filter(e => e.foodBuff).forEach(e => (auraObj[e.varName] = false))
  if (aura.weaponOil)
    Auras.filter(e => e.weaponOil).forEach(e => (auraObj[e.varName] = false))
  ...
  if (aura.demonicRune)
    Auras.filter(e => e.demonicRune).forEach(e => (auraObj[e.varName] = false))
  if (aura.drums)
    Auras.filter(e => e.drums).forEach(e => (auraObj[e.varName] = false))
}
...
export default function AuraSelection() {
  ...
  function auraClickHandler(aura: Aura): void {
    ...
    const isAuraDisabled =
      newAuras[aura.varName] == null || newAuras[aura.varName] === false
    ...
    newAuras[aura.varName] =
      newAuras[aura.varName] == null ? true : !newAuras[aura.varName]
