    ...
    <div id="sidebar">
      <section id="character-section">
        <p id="character-race">
          <span id="race">
            {t(
              Races.find((e) => e.varName === playerState.settings.race)!.name
            )}
          </span>{" "}
          {t("Warlock")}
        </p>
              ...
              <li key={nanoid()} className="sidebar-set-bonus">
                <a
                  ...
                  href={`${getBaseWowheadUrl(i18n.language)}/item-set=${
                    Sets.find((e) => e.setId === set[0])?.setId
                  }`}
                  className={Sets.find((e) => e.setId === set[0])?.quality}
                >
                  {t(Sets.find((e) => e.setId === set[0])!.name)} ({set[1]})
                </a>
              </li>
            ))}
        </ul>
        ...
        <div id="sidebar-simulation-selection">
          ...
          <p id="guybrush-note">
            This sim was heavily inspired by{" "}
            <a
              rel="noreferrer"
              target="_blank"
              href="https://guybrushgit.github.io/WarriorSim/"
            >
              Guybrush's 1.13 Warrior Simulator
            </a>
          </p>
        </div>
      </section>
