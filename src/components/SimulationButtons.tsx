...
function getEquippedMetaGemId(
  ...
  for (const gemArray of Object.values(gems.head[items.head])) {
    if (
      gemArray &&
      Gems.find((e) => e.id === gemArray[1])?.color === GemColor.Meta
    ) {
      return gemArray[1];
    }
  }
...
export function SimulationButtons() {
  ...
  function simulate(simulationParams: {
    ...
    if (
      playerState.settings.maxWebWorkers &&
      parseInt(playerState.settings.maxWebWorkers) > 0
    ) {
            ...
            (params: SimulationEnd) => {
              ...
              if (simulationsFinished === simWorkerParameters.length) {
                ...
                if (simulationParams.type === SimulationType.Normal) {
                  ...
                  if (
                    playerState.settings["automatically-open-sim-details"] ===
                    "yes"
                  ) {
                    dispatch(
                      setCombatLogBreakdownValue({
