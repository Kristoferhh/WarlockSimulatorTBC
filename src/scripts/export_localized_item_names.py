...
def open_wowhead_url_and_get_name(url, driver):
    ...
    for window_handle in driver.window_handles:
        if window_handle != main_window:
            ...
            break
    item_name = WebDriverWait(driver, 30).until(visibility_of_element_located((By.CSS_SELECTOR, wowhead_name_css_selector))).get_attribute("innerHTML").split('<span')[0]
...
with webdriver.Firefox() as driver:
    ...
    for i in range(item_slot_amount):
        ...
        for item in driver.find_elements(By.CSS_SELECTOR, ".item-row-name"):
            ...
            wowhead_url = add_localize_prefix_to_wowhead_url(innerHTML.split('<a href="')[1].split('"></a>')[0])
            ...
            if item_name == "Hood of the Corruptor":
                ...
                for i in range(2):
                    item.find_elements_by_xpath('../td')[2].find_elements(By.CSS_SELECTOR, "div a")[i].click()
                    gems = driver.find_elements(By.CSS_SELECTOR, '.gem-name a')
                    for gem in gems:
                        gem_name = gem.get_attribute('innerHTML')
                        wowhead_url = add_localize_prefix_to_wowhead_url(gem.get_attribute('href'))
                        item_names[gem_name] = open_wowhead_url_and_get_name(wowhead_url, driver)
                    sleep(1)
            ...
            item_names[item_name] = open_wowhead_url_and_get_name(wowhead_url, driver)
